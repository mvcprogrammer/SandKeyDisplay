...
public class ResidentialController : Controller
{
    ...
    public ResidentialController(IConfiguration configuration)
    {
    ...
    {
        skip = SafeSkip(skip);
        var listingsViewModel = await GetListings(AccessToken, sortDirection: sortDirection, skip: skip, condoId: condoId);

        if (listingsViewModel.Count == 0) RedirectToAction("Index", "Home");

        ViewBag.SortDirection = sortDirection;
        ViewBag.Skip = SafeSkip(skip);
    ...
    {
        var listingViewModel = await GetListing(AccessToken, listingKey);
    ...
    {
        var stellarListingsModel = await StellarService.GetListings(accessToken, sortDirection: sortDirection, skip: skip, condoId: condoId, listingType: (int)ParamEnums.ListingType.ResidentialSale);
